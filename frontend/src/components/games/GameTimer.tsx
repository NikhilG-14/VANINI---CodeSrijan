...
export function GameTimer({ durationMs, onExpire, accent = '#7c3aed', label }: GameTimerProps) {
...
export function GlobalTimerBar({ totalMs, startedAt }: GlobalTimerBarProps) {
    ...
    <div className="flex items-center gap-3 w-full">
      ...
      <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
        <div
          ...
          style={{
            width: `${pct * 100}%`,
            background: pct < 0.5 ? '#7c3aed' : pct < 0.75 ? '#f59e0b' : '#ef4444',
          }}
        />
      </div>
    </div>
